...
void jaw_editable_text_interface_init(AtkEditableTextIface *iface,
    ...
    iface->set_run_attributes =
        NULL; // TODO: jaw_editable_text_set_run_attributes, make a proper
              // conversion between attrib_set and swing AttributeSet,
...
void jaw_editable_text_insert_text(AtkEditableText *text, const gchar *string,
    ...
    if (utf8_len >= 0) {
        string_copy = g_strndup(
            string,
            (gsize)utf8_len); // A newly-allocated buffer utf8_len + 1 bytes
