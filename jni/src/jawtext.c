...
static gchar *private_jaw_text_get_gtext_from_string_seq(JNIEnv *jniEnv,
    ...
    if (jniEnv == NULL || jStrSeq == NULL || start_offset == NULL ||
        end_offset == NULL) {
        g_warning("%s: Null argument. jniEnv=%p, jStrSeq=%p, start_offset=%p, "
                  "end_offset=%p",
    ...
    *end_offset = (gint)(*jniEnv)->GetIntField(jniEnv, jStrSeq,
    ...
    if ((*jniEnv)->ExceptionCheck(jniEnv)) {
        jaw_jni_clear_exception(jniEnv);
        g_warning("%s: Failed to get offsets from StringSequence", G_STRFUNC);
        (*jniEnv)->PopLocalFrame(jniEnv, NULL);
        return NULL;
    }
...
static gunichar jaw_text_get_character_at_offset(AtkText *text, gint offset) {
    ...
    if (jcodepoint == 0) {
        g_warning("%s: jcodepoint is 0", G_STRFUNC);
        return 0;
    }
